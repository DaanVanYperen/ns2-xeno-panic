function Shared:ShotgunMessage(chatMessage)
    ...
    Server.SendNetworkMessage("Chat", BuildChatMessage(false, "Xeno Panic Mod", -1, kTeamReadyRoom, kNeutralTeamType, chatMessage), true)
    Shared.Message("Chat All - Xeno Panic Mod: " .. chatMessage)
...
function Shared:ShotgunEgoStroke(player, chatMessagePlayer, chatMessageOthers)
    if (chatMessage == nil) then return end
    Server.SendNetworkMessage("Chat", BuildChatMessage(false, "Xeno Panic Mod", -1, kTeamReadyRoom, kNeutralTeamType, chatMessagePlayer), true)
    Shared.Message("Chat All - Xeno Panic Mod: " .. chatMessage)
    Server.AddChatToHistory(chatMessage, "Shotgun Mod", 0, kTeamReadyRoom, false)
end
...
function Player:ShotgunMessage(chatMessage)
    Server.SendNetworkMessage(self, "Chat", BuildChatMessage(false, "Xeno Panic Mod", -1, kTeamReadyRoom, kNeutralTeamType, chatMessage), true)
end      
